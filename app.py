...
class NexusProductionEngine:
    def __init__(self):
        # Initializing connection with your verified API URL
...
def main():
    # If not authenticated, show the external gate
    if "user" not in st.session_state:
        render_auth_gate()
    else:
        # Switch to the internal command center
            ...
            if st.button("AUTHORIZE ACCESS"):
                try:
                    if mode == "Register Organization":
                        ...
                        st.success("✅ Profile Initialized. If Confirm Email is OFF, go to Login.")
                    else:
                        # Login and Profile Sync
                        auth = nexus.supabase.auth.sign_in_with_password({"email": email, "password": pwd})
                        # Sync ensures no 'duplicate key' errors on login
                    ...
                    st.error("Access Denied: Check credentials or verify 'Confirm Email' is OFF in Supabase.")
...
def render_internal_dashboard():
    ...
    # KPIs as seen in your intelligence center
    ...
    st.subheader("📊 Strategic ROI Analysis")
    st.area_chart(pd.DataFrame(np.random.randn(20, 3), columns=['SEO', 'Authority', 'ROI']))
