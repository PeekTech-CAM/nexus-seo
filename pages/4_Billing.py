"""
Billing & Upgrade Page - Complete Version
"""
...
if not stripe_ready:
    st.error("⚠️ Payment system not configured")
    st.stop()
...
with col1:
    st.metric("Plan", user_profile.get('tier', 'FREE').upper() if user_profile else 'FREE')

with col2:
    st.metric("Credits", f"{user_profile.get('credits_balance', 0):,}" if user_profile else "0")
...
plans = [
    {
        ...
        'price': '€49',
        'period': 'month',
        'price_id': st.secrets.get("STRIPE_PRICE_PRO_MONTHLY", "") if hasattr(st, 'secrets') else "",
    ...
    },
    {
        ...
        'period': 'year',
        'save': 'Save €118/year',
        'price_id': st.secrets.get("STRIPE_PRICE_PRO_ANNUAL", "") if hasattr(st, 'secrets') else "",
    ...
    },
    {
        ...
        'price': '€149',
        'period': 'month',
        'price_id': st.secrets.get("STRIPE_PRICE_AGENCY_MONTHLY", "") if hasattr(st, 'secrets') else "",
    ...
    },
    {
        ...
        'period': 'year',
        'save': 'Save €358/year',
        'price_id': st.secrets.get("STRIPE_PRICE_AGENCY_ANNUAL", "") if hasattr(st, 'secrets') else "",
    ...
    },
    {
        ...
        'price': '€399',
        'period': 'month',
        'price_id': st.secrets.get("STRIPE_PRICE_ELITE_MONTHLY", "") if hasattr(st, 'secrets') else "",
    ...
    },
    {
        ...
        'period': 'year',
        'save': 'Save €488/year',
        'price_id': st.secrets.get("STRIPE_PRICE_ELITE_ANNUAL", "") if hasattr(st, 'secrets') else "",
    ...
    with cols[idx]:
        ...
        if st.button(f"Select {plan['name']}", key=button_key, type=button_type, use_container_width=True):
            if not plan['price_id']:
                st.error(f"⚠️ {plan['name']} not configured yet. Contact support.")
            else:
                with st.spinner("Creating secure checkout session..."):
                    try:
                        checkout_session = stripe.checkout.Session.create(
                            ...
                            mode='subscription',
                            success_url="https://nexus-seo-fobcg4apinvom9hzpjnfyb.streamlit.app/?success=true&plan=" + plan['tier'],
                            cancel_url="https://nexus-seo-fobcg4apinvom9hzpjnfyb.streamlit.app/?cancelled=true",
...
# Second row
if row2_plans:
    cols = st.columns(len(row2_plans))
...
credit_packs = [
    {
        ...
        'price': '€10',
        'credits': 1000,
        'price_id': st.secrets.get("STRIPE_PRICE_CREDITS_1000", "") if hasattr(st, 'secrets') else "",
        'per_credit': '€0.01'
    },
    {
        'name': '5,000 Credits',
        'price': '€40',
        'credits': 5000,
        'price_id': st.secrets.get("STRIPE_PRICE_CREDITS_5000", "") if hasattr(st, 'secrets') else "",
        'badge': '⭐ BEST VALUE',
        'per_credit': '€0.008'
    },
    {
        ...
        'price': '€75',
        'credits': 10000,
        'price_id': st.secrets.get("STRIPE_PRICE_CREDITS_10000", "") if hasattr(st, 'secrets') else "",
        'per_credit': '€0.0075'
    }
]
    ...
    with cols[idx]:
        ...
        if st.button(f"Buy {pack['name']}", key=f"credit_{idx}", use_container_width=True):
            if not pack['price_id']:
                st.error("⚠️ Not configured yet. Contact support.")
            else:
                with st.spinner("Creating checkout..."):
                    try:
                        checkout_session = stripe.checkout.Session.create(
                            ...
                            mode='payment',
                            success_url="https://nexus-seo-fobcg4apinvom9hzpjnfyb.streamlit.app/?success=true&credits=" + str(pack['credits']),
                            cancel_url="https://nexus-seo-fobcg4apinvom9hzpjnfyb.streamlit.app/?cancelled=true",
                            client_reference_id=user_id,
                            customer_email=user_email,
                            metadata={
    ...
    if st.button("🔧 Billing Portal"):
        try:
            portal_session = stripe.billing_portal.Session.create(
                ...
                return_url="https://nexus-seo-fobcg4apinvom9hzpjnfyb.streamlit.app/"
            )
...
st.markdown("---")
st.caption("💳 Secure payments powered by Stripe | 🔒 Your data is encrypted and secure")